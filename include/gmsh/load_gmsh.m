...
%      disp('elements have been read')
 ...
 fclose(fid);
 %%%%%%%%%%%%% AGREG� ESTO %%%%%%%%%%%%%%%%%%%%%%%%
%  disp('remove some fields')
if (mesh.nbPoints==0) 
    s=char('nbPoints', 'POINTS');
    mesh=rmfield(mesh,s) ;
end
if (mesh.nbLines==0) 
    s=char('nbLines', 'LINES');
    mesh=rmfield(mesh,s) ;
end
if (mesh.nbTriangles==0) 
    s=char('nbTriangles', 'TRIANGLES');
    mesh=rmfield(mesh,s) ;
end

if (mesh.nbQuads==0) 
    s=char('nbQuads', 'QUADS');
    mesh=rmfield(mesh,s) ;
end
if (mesh.nbHexas==0) 
    s=char('nbHexas', 'HEXAS');
    mesh=rmfield(mesh,s) ;
end
if (mesh.nbPrisms==0) 
    s=char('nbPrisms', 'PRISMS');
    mesh=rmfield(mesh,s) ;
end
if (mesh.nbPyramids==0) 
    s=char('nbPyramids', 'PYRAMIDS');
    mesh=rmfield(mesh,s) ;
end

if isfield(mesh,'POINTS')
    mesh.POINTS=mesh.POINTS(1:1:mesh.nbPoints,:);
end
if isfield(mesh,'LINES') 
    mesh.LINES=mesh.LINES(1:1:mesh.nbLines,:);
end
if isfield(mesh,'TRIANGLES')
    mesh.TRIANGLES=mesh.TRIANGLES(1:1:mesh.nbTriangles,:);
end
if isfield(mesh,'TETS')
    mesh.TETS=mesh.TETS(1:1:mesh.nbTets,:);
end
% disp('mesh readed')