function T = load_mesh(path)

% T = load_mesh(path)
%
% Input:
%          path: path of the gmsh .msh structure
% Output:
%             T: expanded terahedrization
%
%Last modified: March 1, 2024

malla=load_gmsh(path);
...
y_coordinate = ((T.coordinates(T.faces(:,2),1)-T.coordinates(T.faces(:,1),1)).*...
  (T.coordinates(T.faces(:,3),3)-T.coordinates(T.faces(:,1),3))-...
  (T.coordinates(T.faces(:,2),3)-T.coordinates(T.faces(:,1),3)).*...
  (T.coordinates(T.faces(:,3),1)-T.coordinates(T.faces(:,1),1)));
...
T.area=(1/2)*sqrt( x_coordinate.^2 + y_coordinate.^2 + z_coordinate.^2 );
...
for f=1:4
    ...
    % Give the normals correctly orientated
    normals=bsxfun(@times,normals,T.orientation(:,f));
