function [tauPP, nxPP, nyPP, nzPP, tauDPx, tauDPy, tauDPz, nxDP, nyDP, nzDP, tauDD] = convFace(T, tau, k, formula)
...
block_dof_d2 = @(x) ((x-1)*6*d2*Nnodes +1):(x)*6*d2*Nnodes;

db_resized = zeros([2, 6 * 2 * d2 * Nnodes]);
db_resized(1, block_dof_d2(1), :) = reshape(db, [1, 6 * d2 * Nnodes]);
db_resized(2, block_dof_d2(2), :) = reshape(db, [1, 6 * d2 * Nnodes]);

% - Repetition of the definitition per element
rep_db = repmat(db_resized, Nelts, 1);
rep_db = reshape(rep_db, [2, Nelts, 6*2*d2*Nnodes]);
...
block2=@(x) (1+(x-1)*2*d2):(x*2*d2);    % Face based indexing: 2D
block3=@(x) (1+(x-1)*d3):(x*d3);    % Element based indexing: 3D

% --------------------- Computation <tau*Di,n x Pj> ------------------- %

% Storage reservation for elements
tauDPx = zeros(4*2*d2,d3,Nelts); % Polynomial products x
tauDPy = zeros(4*2*d2,d3,Nelts); % Polynomial products y 
tauDPz = zeros(4*2*d2,d3,Nelts); % Polynomial products z

nxDP =zeros(4*2*d2,d3,Nelts); % X-normal Y-cross
nyDP =zeros(4*2*d2,d3,Nelts); % Y-normal Z-cross
nzDP =zeros(4*2*d2,d3,Nelts); % Z-normal Y-cross

% Normals substructure definition
nxArea = zeros([4, Nelts]);  % X-Component cross
nyArea = zeros([4, Nelts]);  % Y-Component cross
nzArea = zeros([4, Nelts]);  % Z-Component cross
...
for l=1:4
    ...
    % Normals substructure creation
    nxArea(l, :) = (Ny - Nz)'.*T.area(T.facebyele(:, l)') ;
    nyArea(l, :) = (Nz - Nx)'.*T.area(T.facebyele(:, l)') ;
    nzArea(l, :) = (Nx - Ny)'.*T.area(T.facebyele(:, l)') ;
    ...
    db_dim_x = [reshape(db_dimentional(1, block_dof_d2(1), :), [d2, 6*Nnodes, Nelts]);...
                reshape(db_dimentional(1, block_dof_d2(2), :), [d2, 6*Nnodes, Nelts])];
    db_dim_y = [reshape(db_dimentional(2, block_dof_d2(1), :), [d2, 6*Nnodes, Nelts]);...
                reshape(db_dimentional(2, block_dof_d2(2), :), [d2, 6*Nnodes, Nelts])];
    db_dim_z = [reshape(db_dimentional(3, block_dof_d2(1), :), [d2, 6*Nnodes, Nelts]);...
                reshape(db_dimentional(3, block_dof_d2(2), :), [d2, 6*Nnodes, Nelts])];
    ...
    all_products_DP = all_products_DP_x + all_products_DP_y + all_products_DP_z;
    ...
    for mu=1:6
        ...
        % Normals 
        nxmu = (Ny - Nz).*(perm(l,:)==mu);
        nymu = (Nz - Nx).*(perm(l,:)==mu);
        nzmu = (Nx - Ny).*(perm(l,:)==mu);
        ...
        bsxfun(@times, taumu, all_products_DP(block2(mu),block3(l),:));
        ...
        nxDP(block2(l),:, :) = nxDP(block2(l),:, :) + ...
         bsxfun(@times, nxmu, all_products_DP(block2(mu),block3(l),:));
        
        % ------------------------ n y-normal ------------------------ %
        % The ny on the block it's redifined if the 
        % correct permutation it's present
        nyDP(block2(l),:, :) = nyDP(block2(l),:, :) + ...
         bsxfun(@times, nymu, all_products_DP(block2(mu),block3(l),:));
        
        % ------------------------ n z-normal ------------------------ %
        % The nz on the block it's redifined if the 
        % correct permutation it's present
        nzDP(block2(l),:, :) = nzDP(block2(l),:, :) + ...
         bsxfun(@times, nzmu, all_products_DP(block2(mu),block3(l),:));
...
% Definition of nxz integral:
nxPP = kron(nxArea(1,:),pbpb1)+kron(nxArea(2,:),pbpb2)...
      +kron(nxArea(3,:),pbpb3)+kron(nxArea(4,:),pbpb4);

nyPP = kron(nyArea(1,:),pbpb1)+kron(nyArea(2,:),pbpb2)...
      +kron(nyArea(3,:),pbpb3)+kron(nyArea(4,:),pbpb4);

nzPP = kron(nzArea(1,:),pbpb1)+kron(nzArea(2,:),pbpb2)...
      +kron(nzArea(3,:),pbpb3)+kron(nzArea(4,:),pbpb4);

% Reshape on a type (a) matrix
nxPP=reshape(nxPP,[d3,d3,Nelts]);
nyPP=reshape(nyPP,[d3,d3,Nelts]);
nzPP=reshape(nzPP,[d3,d3,Nelts]);
...
% - Dimentional of the basis
% - Dimentional of the basis
block_dof_d2 = @(x) ((x-1)*d2*Nnodes +1):(x)*d2*Nnodes;

db_resized = zeros([2, 6 * 2 * d2 * Nnodes]);
db_resized(1, block_dof_d2(1), :) = reshape(db, [1, d2 * Nnodes]);
db_resized(2, block_dof_d2(2), :) = reshape(db, [1, d2 * Nnodes]);
...
for l=1:4
    ...
    % - Also for each coordiante we introduce:
    db_dim_x = [reshape(db_dimentional(1, block_dof_d2(1), :), [d2, Nnodes, Nelts]);
                reshape(db_dimentional(1, block_dof_d2(2), :), [d2, Nnodes, Nelts])];
    db_dim_y = [reshape(db_dimentional(2, block_dof_d2(1), :), [d2, Nnodes, Nelts]);
                reshape(db_dimentional(2, block_dof_d2(2), :), [d2, Nnodes, Nelts])];
    db_dim_z = [reshape(db_dimentional(3, block_dof_d2(1), :), [d2, Nnodes, Nelts]);
                reshape(db_dimentional(3, block_dof_d2(2), :), [d2, Nnodes, Nelts])];
    ...
    tauDD(block2(l),block2(l),:)= tauDD(block2(l),block2(l),:) + ...
