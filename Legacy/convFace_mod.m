function [tauPP,...
          ...
          nxDP, nyDP, nzDP,...
          tauDD_x, tauDD_y, tauDD_z] = convFace_mod(T, tau, k, formula)
...
nxDP =zeros(4*d2,d3,Nelts); % X-normal
nyDP =zeros(4*d2,d3,Nelts); % Y-normal
nzDP =zeros(4*d2,d3,Nelts); % Z-normal
...
for l=1:4
    ...
    % Definition of all cross products
    ...
    for mu=1:6
        ...
        % Tau components
        taumu=reshape(TauArea(l,:).*(perm(l,:)==mu), [1, 1, Nelts]);

        % Normals 
        nxmu = reshape(Nx.*(perm(l,:)==mu), [1, 1, Nelts]);
        nymu = reshape(Ny.*(perm(l,:)==mu), [1, 1, Nelts]);
        nzmu = reshape(Nz.*(perm(l,:)==mu), [1, 1, Nelts]);
    ...
    bsxfun(@times, nymu - nzmu, all_products_DP_x(block2(mu),block3(l),:));
        
        % ------------------------ n y-normal ------------------------ %
        % The ny on the block it's redifined if the 
        % correct permutation it's present
        nyDP(block2(l),:, :) = nyDP(block2(l),:, :) + ...
    bsxfun(@times, nzmu - nxmu, all_products_DP_y(block2(mu),block3(l),:));
        
        % ------------------------ n z-normal ------------------------ %
        % The nz on the block it's redifined if the 
        % correct permutation it's present
        nzDP(block2(l),:, :) = nzDP(block2(l),:, :) + ...
    bsxfun(@times, nxmu - nymu, all_products_DP_z(block2(mu),block3(l),:));
