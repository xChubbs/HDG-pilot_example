...
switch n
    ...
    case 2
        ...
        example=4;
        ctmass=1;
        domain=2;   % ALR: Modification on domain >> Original domain = 1
    case 3
        k=2;
        example=4;
        ctmass=1;
...
switch domain
    ...
    case 2
        % ALR: Modification on domain >> Cube to validate
        %load sixT3dDir                               % (0,1)^3 all Dirichlet
        %matObj = matfile('Drivers/');                % Load of .mat object
        %listT  = {matObj.T1, matObj.T2, matObj.T3};  % List of Stuctures available

        % Importing of a singular structure for first case scenario: Unitary cube
        T1 = load_mesh('cubeD0.msh'); T2 = load_mesh('cubeD1.msh'); 
        T3 = load_mesh('cubeD3.msh'); T4 = load_mesh('cubeD4.msh');
        % T5 = load_mesh('cubeD4.msh');

        % Listing of elements for growth of method testing
        listT = {T1, T2, T3, T4};

    case 3
        load Corner        % Fichera corner (uniform partition) - mixex BC
