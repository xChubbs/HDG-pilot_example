function [tauPP, nPP, tauDP, tauDP_t, nDP, tauDD] = matricesFace(T, tau, k, formula)
...
for l=1:4
    ...
    Nx = T.orientation(:, l).*T.normals(:,3*(l-1)+1);
    Ny = T.orientation(:, l).*T.normals(:,3*(l-1)+2);
    Nz = T.orientation(:, l).*T.normals(:,3*(l-1)+3);

    Nx = reshape(Nx, [1, 1, Nelts]);
    Ny = reshape(Ny, [1, 1, Nelts]);
    Nz = reshape(Nz, [1, 1, Nelts]);
...
for l=1:4
    ...
    % Selection of skeleton directors
    A_0 = reshape(T.A(1, :, T.facebyele(:, l)), [3, Nelts]);
    A_1 = reshape(T.A(2, :, T.facebyele(:, l)), [3, Nelts]);
