...
\noindent Finalmente estas componentes de la matriz $\boldsymbol{nDP_e}$ se organizan en matrices de bloques, cada una de tamaño $d_3 \times 2 d_2 \times \text{Nelts}$ haciendo referencia a las componentes $\lbrace x, y, z \rbrace$ de las variables \textit{(dimensión $d_3$)}, dimensión de la base de cada cara \textit{(dimensión $2d_2$)} y elementos \textit{(dimensión $\text{Nelts}$)}, tal como se muestra en la figura \ref{matrix3d}. La matriz $\boldsymbol{nDP}$ tiene dimensiones $(3 \times d_3) \times (4 \times 2 d_2) \times \text{Nelts}$ debido a cada una de las componentes y las cuatro caras que tiene cada elemento.
